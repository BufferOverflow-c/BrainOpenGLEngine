...
static const char *defaultParticleVertexShader = GL2D_OPNEGL_SHADER_VERSION
...
#pragma region newParticles
  ...
  //		color[i].x = rand({ ps.createApearence.color1.x,
  // ps.createApearence.color2.x }); 		color[i].y = rand({
  // ps.createApearence.color1.y, ps.createApearence.color2.y });
  // color[i].z = rand({ ps.createApearence.color1.z,
  // ps.createApearence.color2.z }); 		color[i].w = rand({
  //ps.createApearence.color1.w, ps.createApearence.color2.w });
  // rotationSpeed[i] = rand(ps.rotationSpeed); 		rotationDrag[i] =
  //rand(ps.rotationDrag); 		deathRattle[i] = 1;
  //
...
void ParticleSystem::emitParticleWave(ParticleSettings *ps, glm::vec2 pos) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (recreatedParticlesThisFrame < ps->onCreateCount && sizeXY[i] == 0) {
      ...
      dragY[i]  = rand(ps->dragY);
      color[i].x =
          rand({ps->createApearence.color1.x, ps->createApearence.color2.x});
      color[i].y =
          rand({ps->createApearence.color1.y, ps->createApearence.color2.y});
      color[i].z =
          rand({ps->createApearence.color1.z, ps->createApearence.color2.z});
      color[i].w =
          rand({ps->createApearence.color1.w, ps->createApearence.color2.w});
...
void ParticleSystem::draw(Renderer2D &r) {
  ...
  if (postProcessing) {
    ...
    if (fb.texture.GetSize() !=
        glm::ivec2{w / pixelateFactor, h / pixelateFactor}) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    float lifePerc =
        duration[i] / durationTotal[i]; // close to 0 when gone, 1 when full
    ...
    case gl2d::TRANZITION_TYPES::wave:
      lifePerc =
          (std::cos(lifePerc * 5 * 3.141592) * lifePerc + lifePerc) / 2.f;
      break;
    case gl2d::TRANZITION_TYPES::wave2:
      lifePerc =
          std::cos(lifePerc * 5 * 3.141592) * std::sqrt(lifePerc) * 0.9f + 0.1f;
      break;
    case gl2d::TRANZITION_TYPES::delay:
      lifePerc =
          (std::cos(lifePerc * 3.141592 * 2) * std::sin(lifePerc * lifePerc)) /
          2.f;
      break;
    case gl2d::TRANZITION_TYPES::delay2:
      lifePerc =
          (std::atan(2 * lifePerc * lifePerc * lifePerc * 3.141592)) / 2.f;
      break;
    default:
      break;
    }
    ...
    if (thisParticleSettings[i]) {
      pos.x = posX[i];
      pos.y = posY[i];
      pos.z = interpolate(sizeXY[i],
                          thisParticleSettings[i]->createEndApearence.size.x,
                          lifePerc);
      pos.w = pos.z;

      c.x = interpolate(color[i].x,
                        thisParticleSettings[i]->createEndApearence.color1.x,
                        lifePerc);
      c.y = interpolate(color[i].y,
                        thisParticleSettings[i]->createEndApearence.color1.y,
                        lifePerc);
      c.z = interpolate(color[i].z,
                        thisParticleSettings[i]->createEndApearence.color1.z,
                        lifePerc);
      c.w = interpolate(color[i].w,
                        thisParticleSettings[i]->createEndApearence.color1.w,
                        lifePerc);
    } else {
      pos.x = posX[i];
      pos.y = posY[i];
    ...
    if (postProcessing) {
      ...
      // r.currentCamera.position += glm::vec2{w / (pixelateFactor * 2.f), h /
      // (pixelateFactor*2.f)}; r.currentCamera.position *= pixelateFactor; c.w
      // = sqrt(c.w);
      //  c.w = 1;
    } else {
      p = pos;
...
void initgl2dParticleSystem() {
  defaultParticleShader = createShaderProgram(defaultParticleVertexShader,
                                              defaultParcileFragmentShader);
}

void cleanupgl2dParticleSystem() { glDeleteShader(defaultParticleShader.id); }
